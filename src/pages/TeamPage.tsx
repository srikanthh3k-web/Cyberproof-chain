...
export default function TeamPage() {
  ...
  useEffect(() => {
    fetch('/api/team').then((r) => r.json()).then((d) => setUsers(d));
  }, []);
      ...
      <div className="grid-3">
        {users.map((user) => (
          <Card key={user.email} style={{ padding: 18 }}>
            <div className="avatar" style={{ width: 42, height: 42, marginBottom: 10 }}>{user.name.split(' ').map((n) => n[0]).join('').slice(0,2)}</div>
